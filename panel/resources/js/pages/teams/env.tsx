...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Team Environment', href: '/team/env' },
];
...
export default function TeamEnv({ vars: initialVars }: { vars: EnvironmentVariable[] }) {
    ...
    const fetchVars = async () => {
        const res = await fetch('/team/env', { headers: { Accept: 'application/json' } });
    ...
    const handleAdd = async () => {
        ...
        setFormErrors({});
        try {
            const res = await fetch('/team/env', {
                method: 'POST',
    ...
    const handleEdit = async () => {
        ...
        setFormErrors({});
        try {
            const res = await fetch(`/team/env/${editingVar.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-XSRF-TOKEN': getCookie('XSRF-TOKEN') },
    ...
    const handleDelete = async () => {
        ...
        await fetch(`/team/env/${deletingVar.id}`, {
            method: 'DELETE',
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">Team Environment Variables</h1>
                        <p className="text-muted-foreground text-sm">Shared across all apps in your team. App-level variables override team-level on key collision.</p>
                    </div>
                </div>
